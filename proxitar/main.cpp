...
int main(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		for (unsigned int uFailCount=0;uFailCount<10;)
		{
			...
			if (!vcChatMessages.empty())
			{
				for (std::vector<ventrilo_message>::iterator itMessage=vcChatMessages.begin();itMessage!=vcChatMessages.end();++itMessage)
				{
					...
					if (ss >> strTemp >> strCmd && string_to_lower(strTemp) == "bot:")
					{
						...
						else if (strCmd == "nig" && objMonitor.IsAdmin(objMessage.strName))
						{
							objChat.Send("I'll beat the fucking black off of you niggers if you don't stop arguing like bitches.");
							os_speechdelay(2500);
						}
			...
			if (!strMessage.empty())
			{
				std::string strFaggotClanName="wwwwwwwwwwwwwwwwwwwwwwwww";
				std::string::size_type pos=strMessage.find(strFaggotClanName);
				if (pos != std::string::npos)
				{
					strMessage.replace(pos,strFaggotClanName.size(),"shove your black dick in my ass");
				}	
				objChat.Send(strMessage);
